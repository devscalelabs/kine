...
import { formatStepResult } from "../utils/formatting";

export function formatBeautifiedResponse(rawResponse: AgentRuntime): string {
	const { response, steps, usage } = rawResponse;
...
export function formatSummary(rawResponse: AgentRuntime): string {
	const { response, steps, usage } = rawResponse;
...
export function formatSteps(rawResponse: AgentRuntime): string {
	const { steps } = rawResponse;
	...
	return steps
		.map((step, index) => {
			...
			if (step.result) {
				const resultStr = formatStepResult(step.result);
				stepStr += `\n  Result: ${resultStr}`;
			}
			if (step.meta?.tokens) {
